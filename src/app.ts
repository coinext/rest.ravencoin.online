...
const ravencoinZmqDecoder = new RavencoinZMQDecoder(process.env.NETWORK)
...
sock.on("message", (topic: any, message: string) => {
  ...
  if (decoded === "rawtx") {
    const txd = ravencoinZmqDecoder.decodeTransaction(message)
    io.emit("transactions", JSON.stringify(txd, null, 2))
  } else if (decoded === "rawblock") {
    const blck = ravencoinZmqDecoder.decodeBlock(message)
